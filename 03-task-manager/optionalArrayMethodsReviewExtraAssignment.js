...
const lastNamesChallenge = (array) =>
  array.map((name) => name.split(" ").pop());
console.log("lastNamesChallenge:", lastNamesChallenge(names));

const formatChallenge = (array) =>
  array.filter((name) => name.split(" ").length === 2);
console.log("formatChallenge:", formatChallenge(names));

const titleCaseChallenge = (array) =>
  array.map((name) => {
    //* ``** Splits the first, middle, and last name of a person
    const splitName = name.split(" ");

    //* ``** Capitalizes the first letter of each first, middle, and last name
    const capitalizeNames = splitName.map((name) => {
      //* ``** Splits all the letters of a name
      const splitLetters = name.split("");
      //* ``** Makes all letters lowercase
      const lowerCaseLetters = splitLetters.map((letter) =>
        letter.toLowerCase()
      );
      //* ``** Grabs just the first letter
      const firstLetter = lowerCaseLetters.slice(0, 1);
      //* ``** Capitalizes the first letter
      const capitalizeFirstLetter = firstLetter.map((letter) =>
        letter.toUpperCase()
      );
      //* ``** Grabs all letters except the first letter
      const allLettersExceptTheFirstOne = lowerCaseLetters.slice(1);
      //* ``** Joins the newly capitalized first letter with the rest of the lowercase letters
      const joined = capitalizeFirstLetter.concat(allLettersExceptTheFirstOne);
      //* ``** Joins all the letters back together
      const wholeNames = joined.join("");
      return wholeNames;
    });
    //* ``** Removes any empty spaces between names
    const noSpaces = capitalizeNames.filter((item) => item !== "");
    //* ``** Joins the first, middle, and last names back together
    const joinNames = noSpaces.join(" ");
    return joinNames;
  });
console.log("titleCaseChallenge:", titleCaseChallenge(names));

const finalChallenge = (array) => {
  const correctFormat = formatChallenge(array);
  const titleCase = titleCaseChallenge(correctFormat);
  const doesNotEndWithZ = titleCase.filter((name) => !name.endsWith("z"));
  const signUpMessage = doesNotEndWithZ.map(
    (name) => `Dear ${name}, please sign up. Thanks!`
  );
  return signUpMessage;
};
console.log("finalChallenge:", finalChallenge(names));
